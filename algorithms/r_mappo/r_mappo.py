...
class R_MAPPO():
    ...
    def __init__(self,
        ...
        if self.enable_res_hu:
            print(f"[HU-Res-MAPPO] Trainer initialized:")
            print(f"  - Sub-update PPO epochs: {self.sub_ppo_epoch}")
            print(f"  - Sub-update LR scale: {self.sub_lr_scale}")
        # ==================== HU-Res-MAPPO 新增配置结束 ====================
    ...
    def train(self, buffer, update_actor=True):
        ...
        # ==================== HU-Res-MAPPO 核心修改：全局更新后重置偏置 ====================
        if self.enable_res_hu:
            # 获取重置前的偏置统计信息（用于日志）
            if hasattr(self.policy.actor, 'get_bias_info'):
                bias_info_before_reset = self.policy.actor.get_bias_info()
                train_info['bias_norm_before_reset'] = bias_info_before_reset.get('bias_norm', 0)
                train_info['bias_max_before_reset'] = bias_info_before_reset.get('bias_max', 0)

            # 重置动作偏置为零
            if hasattr(self.policy.actor, 'reset_action_bias'):
                self.policy.actor.reset_action_bias()
                print("[HU-Res-MAPPO] Global update completed, action_bias reset to zero")
        # ==================== HU-Res-MAPPO 核心修改结束 ====================
    ...
    def sub_train(self, buffer, sub_start, sub_end):
        ...
        关键逻辑：
        1. 冻结主网络参数（base, rnn等），只允许action_bias更新
        2. 执行PPO更新（梯度只流向action_bias）
        3. 解冻所有参数（为全局更新做准备）

        :param buffer: (SharedReplayBuffer) replay buffer
        :param sub_start: (int) 子episode起始step
        :param sub_end: (int) 子episode结束step（不含）
        :return train_info: (dict) 训练统计信息
        """
        # ==================== Step 1: 冻结主网络参数 ====================
        frozen_params_count = 0
        trainable_params_count = 0

        if self.enable_res_hu and hasattr(self.policy.actor, 'action_bias'):
            ...
            print(f"[HU-Res-MAPPO Sub-Update] Parameter freeze status:")
            print(f"  - Frozen params: {frozen_params_count:,}")
            print(f"  - Trainable params (bias only): {trainable_params_count}")
        # ==================== Step 1 结束 ====================

        # ==================== Step 2: 计算子episode的优势函数 ====================
        # 注意：这部分计算应该在smac_runner.py的sub_update中完成
        # 这里假设buffer已经包含了正确的returns和advantages

        # 获取子episode范围的数据
        # 计算优势函数（只使用sub_start到sub_end的数据）
        if self._use_popart or self._use_valuenorm:
        ...
        for _ in range(sub_ppo_epoch):
            ...
            for sample in data_generator:
                ...
                train_info['sub_actor_grad_norm'] += actor_grad_norm
        ...
        if self.enable_res_hu:
            ...
            # 记录偏置更新后的统计信息
            if hasattr(self.policy.actor, 'get_bias_info'):
                bias_info = self.policy.actor.get_bias_info()
                train_info['sub_bias_norm'] = bias_info.get('bias_norm', 0)
                train_info['sub_bias_mean'] = bias_info.get('bias_mean', 0)
                train_info['sub_bias_max'] = bias_info.get('bias_max', 0)

            print(f"[HU-Res-MAPPO Sub-Update] Completed, all params unfrozen")
            print(f"  - Sub policy loss: {train_info['sub_policy_loss']:.4f}")
            print(f"  - Sub ratio: {train_info['sub_ratio']:.4f}")
            if 'sub_bias_norm' in train_info:
                print(f"  - Bias norm: {train_info['sub_bias_norm']:.4f}")
        # ==================== Step 4 结束 ====================
